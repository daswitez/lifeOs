...
export default function AccionesPage() {
  const inboxTasks = [
    "Pensar idea sobre nueva estructura de base de datos",
    "Llamar al seguro del auto",
  ];

  const todayTasks = [
    { title: "Filtrar candidatos para UX", ctx: "work", priority: true },
    { title: "Terminar reporte semanal Q3", ctx: "work", priority: false },
    { title: "Comprar pasajes a Berlín", ctx: "life", priority: false },
  ];
  ...
  return (
    <div className="min-h-full pb-24">
      ...
      <header className="sticky top-0 z-10 glass-header px-8 md:px-16 py-6 flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-[var(--foreground)]">Acciones</h1>
      ...
      <div className="px-8 md:px-16 mt-8 max-w-4xl">
        ...
        <section className="mb-12">
          <h2 className="flex items-center gap-2 text-sm font-semibold text-[var(--muted-foreground)] mb-4 tracking-widest uppercase">
            <InboxIcon className="w-4 h-4" /> Bandeja de Entrada (2)
          </h2>
          ...
          <div className="flex flex-col border border-[var(--border)] rounded-xl overflow-hidden bg-[var(--card)] shadow-sm">
            {todayTasks.map((task, idx) => (
              ...
              >
                ...
                {/* Metadatos alineados a la derecha de forma sutil */}
